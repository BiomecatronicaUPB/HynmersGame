...
void UKinectSensor::InitializeSensor(){

	DWORD dwFlags = NUI_INITIALIZE_FLAG_USES_COLOR | NUI_INITIALIZE_FLAG_USES_DEPTH_AND_PLAYER_INDEX | NUI_INITIALIZE_FLAG_USES_SKELETON;
	...
	if (NULL != mNuiSensor)
	{
		...
		if (SUCCEEDED(hr))
		{
			UE_LOG(KinectLog, Log, TEXT("Sensor initialized"));
			
			if (SUCCEEDED(OpenColorStream())){
				UE_LOG(KinectLog, Log, TEXT("Color Stream open"));
			}
			if (SUCCEEDED(OpenDepthStream())){
				UE_LOG(KinectLog, Log, TEXT("Depth Stream open"));
			}
			if (SUCCEEDED(OpenSkeletonStream())){
				UE_LOG(KinectLog, Log, TEXT("Skeleton Stream open"));
			}
...
void UKinectSensor::UpdateSkeleton(FKinectControllerState * KinectControllerStates){
	if (NULL == mNuiSensor)
	{
		return;
	}
	...
	{
		ProcessSkeleton(KinectControllerStates);
	}
}
...
void UKinectSensor::ProcessSkeleton(FKinectControllerState * KinectControllerStates){
	...
	for (int i = 0; i < NUI_SKELETON_COUNT; i++)
	{
		...
		FSkelStruct newSkelStruc;
		newSkelStruc.SetPlayerID(i);
		...
		if (NUI_SKELETON_TRACKED == trackingState)
		{
			...
			for (int p = 0; p < NUI_SKELETON_POSITION_COUNT; ++p)
			{
				...
				FVector JointPosition(hh.z, hh.x*-1.0, hh.y);
				JointPosition.Normalize();
				TempJointPosition.Add(JointPosition);
			...
			KinectControllerStates->TimeStamp.Scale = FPlatformTime::ToSeconds(FPlatformTime::Cycles());

			newSkelStruc.SetSkelJointPosition(TempJointPosition);
			newSkelStruc.SetSkelRotationAbsoluteQuaternion(TempJointAbsoluteRotation);
			newSkelStruc.SetSkelRotationHierarchicalQuaternion(TempJointHierarchicalRotation);
			Skeletons=newSkelStruc;
		}
